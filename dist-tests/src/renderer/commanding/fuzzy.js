...
export function fuzzyMatch(query, items, accessor, extraWeight) {
    const q = query.trim().toLowerCase();
    if (!q)
        return items.slice(0, 100).map(i => ({ item: i, score: Number.MAX_SAFE_INTEGER, highlights: [] }));
    const results = [];
